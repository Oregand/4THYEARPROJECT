...
print " "
with open('../DataMining/vw_golfDealz.txt', 'w') as outfile:
    for i in range(50):
        outputPrice = predictedPrice.predict(predictedPrice.transform(df.T.to_dict()[i]))
        print outputPrice
        json.dump(outputPrice, outfile)
        outfile.write('\n')

# --------------------------------------------------------------------------------------------------------
# Insert my deals into my MYSQL DB "Dealz" Table,
